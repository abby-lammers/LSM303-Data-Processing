...
*/
  ...
  Ym_raw = (ylo | (yhi << 8));
  Zm_raw = (zlo | (zhi << 8));   
//To calculate the value you need to devide X and Y by 980 gauss⁻¹, it's 1100 gauss⁻¹ for Z

 // I NEVER got the temp sensor working correctly? seems to have a large offset?! http://forums.adafruit.com/viewtopic.php?f=50&t=36212&start=0&hilit=lsm303  
 /* 
  //Read Mag temperature - did you set Temp_en=1 to enable during init?
  //TEMP_OUT_H_M (31h), TEMP_OUT_L_M (32h)  
  //Temperature (12bit)  = (TEMP_OUT_L_M | (TEMP_OUT_H_M << 8))>>4;
  //The value is expressed as 2’s complement. 
  Wire.beginTransmission((byte)LSM303_ADDRESS_MAG);
  Wire.write(LSM303_REGISTER_MAG_TEMP_OUT_H_M | 0b10000000);
  Wire.endTransmission();
  Wire.requestFrom((byte)LSM303_ADDRESS_MAG, (byte)2);  
  while (Wire.available() < 2);
  xhi = Wire.read();
  xlo = Wire.read();  
  LSM303_TEMP_Raw = (int16_t)((xhi << 8 | xlo)>>2);   //(8LSB/deg)
  // Sleep temp sensor         CRA_REG_M = [ TEMP_EN  |0|0|  DO2 | DO1 | DO0  |0|0 ] 
  i2c_writeRegByte(LSM303_ADDRESS_MAG, LSM303_REGISTER_MAG_CRA_REG_M, 0b00001100); //Temp_en=0 to disable  
 */
...
//NOW adjust the readings with the offsets!

// n0m1 simply does this: to offset compass by hard iron
  // raw.XAxis += 36;
  // raw.YAxis += 68;
  // raw.ZAxis += 312;  
  
// and freeIMU does this:  
// CalibratedData = (unCalibratedData -Offset)/Scaling Factor

/*  all the variabls for the corrections should be stored at the beginning of the script...
//  Not sure if I should even bother doing these calcs on the arduino!

//but Magneto does this for the Lsm303dlhc: (100000/1100 scaling factor to match total field values)
Xm_off = Xm_raw*(100000.0/1100.0) - Xm_bias; //X-axis combined bias (Non calibrated data - bias)
Ym_off = Ym_raw*(100000.0/1100.0) - Ym_bias; //Y-axis combined bias (Default: substracting bias)
Zm_off = Zm_raw*(100000.0/980.0 ) - Zm_bias; //Z-axis combined bias

Xm_cal =  Xm_x*Xm_off + Xm_y*Ym_off + Xm_z*Zm_off; //X-axis correction for combined scale factors (Default: positive factors)
Ym_cal =  Ym_x*Xm_off + Ym_y*Ym_off + Ym_z*Zm_off; //Y-axis correction for combined scale factors
Zm_cal =  Zm_x*Xm_off + Zm_y*Ym_off + Zm_z*Zm_off; //Z-axis correction for combined scale factors

// Magneto Accelerometer calibration
Xa_off = Xa_raw - Xa_bias;
Ya_off = Ya_raw + Ya_bias;
Za_off = Za_raw + Za_bias;
Xa_cal =  Xa_x*Xa_off + Xa_y*Ya_off + Xa_z*Za_off; //X-axis correction for combined scale factors (Default: positive factors)
Ya_cal =  Ya_x*Xa_off + Ya_y*Ya_off + Ya_z*Za_off; //Y-axis correction for combined scale factors
Za_cal =  Za_x*Xa_off + Za_y*Ya_off + Za_z*Za_off; //Z-axis correction for combined scale factors


lsm303MagX = (int)Xm_cal/100; //this did not fix anything
lsm303MagY = (int)Ym_cal/100;
lsm303MagZ = (int)Zm_cal/100; //somehow the sign is being fipped here??

  Serial.println (F(""));
  Serial.print (F("  Xa_cal: "));
  Serial.print (Xa_cal); 
  Serial.print (F("  Ya_cal: "));
  Serial.print (Ya_cal);      
  Serial.print (F("  Za_cal: "));
  Serial.println (Za_cal); 

  Serial.println (F(""));
  Serial.print (F("  lsm303MagX: "));
  Serial.print (lsm303MagX); 
  Serial.print (F("  lsm303MagY: "));
  Serial.print (lsm303MagY);      
  Serial.print (F("  lsm303MagZ: "));
  Serial.println (lsm303MagZ);  

//Then do the calculations of roll pitch yaw
//enter compass data and accel data for calculation
compCompass(lsm303MagX,-(lsm303MagY),lsm303MagZ,Xa_cal,-(Ya_cal),Za_cal, false);  //  why the sign changes?   //The default order is (+x, +y, +z).
// freescale guide for signs:http://www.freescale.com/files/sensors/doc/app_note/AN4248.pdf  
//This application note uses the industry standard yaw-pitch-roll ordering in aerospace NED coordinate systems
1. roll (clockwise around the north-pointing horizontal axis)
2. pitch (clockwise around the east-pointing horizontal axis)
3. heading (around the vertical axis). A positive yaw angle ψ (azimuth) is defined to be a clockwise rotation about the positive z-axis.

// gets roll pitch and yaw working when compass is level, but goes completely nuts when board is tilted!

// compCompass(lsm303MagX,lsm303MagY,lsm303MagZ,smoothACCx,smoothACCy,smoothACCz, false); 
// https://github.com/n0m1/Comp6DOF_n0m1  
  Serial.print (F("Roll: ")); //returns roll result -18000 to 18000  //roll_ = atan2Int(yAccel, zAccel);
  Serial.print (roll_/100); 
  Serial.print (F("  Pitch: ")); //returns yaw result -18000 to 18000  //pitch_ = atan2Int((int)-xAccel, zAccel);
  Serial.print (pitch_/100);      
  Serial.print (F("  Yaw: "));  //yaw_ = atan2Int((int)-yMagAxisComp_, xMagAxisComp_); 
  Serial.print (yaw_/100);   //returns yaw result -18000 to 18000 
  
//float compHeading = atan2Int(yMagAxisComp_,xMagAxisComp_);  //realHeading = atan2(-YMagcompensated, XMagcompensated) * 360.0 / (2*PI);
//atan2 returns -PI to PI or -180 to 180 degrees. If you want the range to be 0-360, add 180
float compHeading = -yaw_ /100; // for east side, clockwise
//if (yaw_>0){  // for west side, inverting the counter clockwise 180 deg output of atan
//  compHeading = 180+(180-(yaw_/100));
//}

float NonCompHeading = (atan2Int(-(lsm303MagY), lsm303MagZ));
  Serial.print (F("  Heading: "));
  Serial.print (compHeading); 
  Serial.print (F("  NonCompHeading: "));
  Serial.println (NonCompHeading/100);   
...
// If you measure the angle from the X direction, then: The angle of this projection is arccos(gx/g'), i.e. arc-cosine of the 
// ratio between the X component of the gravity pull vector and the magnitude of its projection onto the XY plane
//angle of this projection = arccos[gx / sqrt(gx^2 + gy^2)] 
 
//float tempI = ((float)Xa_raw/(float)Gproj_2_XYplane);
//By default, arduino will divide integers using integer math, so 5/15 will equal 0.  

// Serial.print (F("tempI: "));
// Serial.println (tempI);  
 
// we have an atan function, but not arccos
// re-expressing arcos in terms of atan http://mathforum.org/library/drmath/view/54020.html
// arcos(x) = arctan((sqrt(1-x^2))/x). but only if 0<x<1!

// so Angle_of_Gproj=atan2Int((sqrt(1-sq((long)tempInt))),tempInt);
//   Angle_of_Gproj=atan2Int(sqrt(1-sq(tempI))*1000,tempI*1000);

//Angle_of_Gproj=atan2Int((sqrt(1-sq(tempI)))*1000,tempI*1000); // no quadrant info, because both become positive? 
//rotation expressed from "south" rather than north?

// Serial.print (F("Z tilt angle: "));
// Serial.print (tiltAngle/100);  
// Serial.print (F("  Rotation (clckwise from X): "));
// Serial.println (Angle_of_Gproj/100); 
 // ? but perhaps this is the angle in the tilted xy plane rather than the world coordinates? need to check this!
